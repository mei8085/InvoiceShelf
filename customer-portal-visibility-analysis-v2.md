...
**关键缺陷分析**：
- 第 18 行 `Invoice::find()` 可能返回 `null`（发票已删除）
- 第 21 行的条件判断 `$invoice && ...` 会跳过状态更新（因为 `$invoice` 是 `null`）
- 第 43 行或第 47-48 行会直接调用 `$invoice->xxx`，**触发 PHP Fatal Error**
- 错误类型：`Error: Call to a member function getGeneratedPDFOrStream() on null` 或 `Error: Attempt to read property "company_id" on null`

#### 3.3.2 估价单 PDF 控制器（同类问题）
...
**同类缺陷**：
- 第 41 行 `$estimate->getGeneratedPDFOrStream('estimate')` 在 `$estimate` 为 `null` 时调用，触发 PHP Fatal Error

#### 3.3.3 付款单 PDF 控制器（不同实现方式，同类问题）
...
**关键差异**：
- 使用 Eloquent 多态关联动态属性 `$emailLog->mailable`
- 如果关联记录已删除，`mailable` 返回 `null`
- 第 16 行直接调用方法，**同样触发 PHP Fatal Error
- 错误类型：`Error: Call to a member function getGeneratedPDFOrStream() on null`
...
| 控制器方法 | 代码模式 | 记录不存在时的行为 | 错误类型 |
|-----------|---------|------------------|---------|
| `InvoicePdfController::getPdf()` | `Model::find()` + 直接调用方法/属性 | ❌ PHP Fatal Error | `Call to a member function getGeneratedPDFOrStream() on null` |
| `EstimatePdfController::getPdf()` | `Model::find()` + 直接调用方法 | ❌ PHP Fatal Error | `Call to a member function getGeneratedPDFOrStream() on null` |
| `PaymentPdfController::getPdf()` | `$emailLog->mailable` + 直接调用方法 | ❌ PHP Fatal Error | `Call to a member function getGeneratedPDFOrStream() on null` |
...
**隐藏风险**：
- `isExpired()` 方法内部调用 `$this->mailable()->get()` 也会在 `mailable_id` 不存在时触发类似问题
- 当 Token 关联的业务记录已删除时，`isExpired()` 也会抛出 PHP Error

### 3.6 Token 链路鉴权条件总结（修正版）

| 校验点 | 位置 | 失败结果 | 备注 |
|-------|------|---------|------|
| Token 存在（路由模型绑定） | Laravel 隐式绑定 | 404 Not Found | EmailLog 记录不存在 |
| `automatically_expire_public_links == NO` | 跳过过期检查 | ✅ 可访问 | 全局关闭过期检查 |
| Token 未过期（创建时间 + N 天） | `EmailLog::isExpired()` | 403 Link Expired | 依赖关联记录存在 |
| `mailable_id` 对应记录存在 | 控制器内查找 | ❌ PHP Fatal Error | 三个 getPdf 方法无空值检查 |
...
| **记录不存在处理** | ✅ 正常查询返回空集合 | ❌ getPdf 触发 PHP Error |
...
5. **PHP Error 风险**：关联记录删除后访问会触发 500 错误，可能泄露服务器信息
...
| **关联记录删除** | 发票/估价单/付款单被删除 | 对应 Token | 永久失效（触发 PHP Error） |
...
public function isExpired()
{
    ...
    // 记录不存在时会触发 Undefined array key 0 错误
    // ...
}
